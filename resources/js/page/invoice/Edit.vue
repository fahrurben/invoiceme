    ...
    export default {
        ...
        data() {
            ...
            let formDefault = {
                ...invoice,
                error: "",
                issueDate: moment(invoice.issueDate?.date).format('YYYY-MM-DD'),
                dueDate: moment(invoice.dueDate?.date).format('YYYY-MM-DD'),
            }

            let linesDefault = []
            for(let line of invoice.lines) {
                let item = _.find(window.arrItem, (item) => item.id == line.item.id)
                let itemName = item != null ? item.name : ""
                linesDefault.push({
                    ...line,
                    itemId: line.item.id,
                    itemName
        ...
        methods: {
            ...
            async save() {
                ...
                let that = this
                try {
                    response = await axios.post(that.baseUrl + "/invoice/update/"+this.invoice.id,{
                        ...this.form,
                        tax: this.tax,
                        lines: [ ...this.lines ]
                    })
